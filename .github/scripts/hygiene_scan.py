...
network addresses belong to `secret_scan.py`. This scanner matches none of them,
so no line is ever failed by both checks; `test_hygiene_scan.py` asserts that
...
Usage:
    python .github/scripts/hygiene_scan.py [path]   # default: current directory
...
SELF = {
    ".github/scripts/hygiene_scan.py",
    ".github/scripts/test_hygiene_scan.py",
}
...
def symlink_finding(root: Path, rel: str) -> Finding | None:
    ...
    climbs out of the repository, describes a layout no clone has."""
    full = root / rel
...
def file_text(root: Path, rel: str) -> str | None:
    """The file's text, or None when it is binary. Raises when it cannot be read
    or decoded; the caller turns that into a finding, never into a pass."""
    try:
...
def scan(root: Path, rules: list[Rule] | None = None,
         symlinks: bool = True) -> list[Finding]:
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        return 2
    if findings:
        print("::error::hygiene: this tree carries traces of the machine it was "
              "written on:\n", file=sys.stderr)
